...
NobleBindings.prototype.init = function() {
    this._onAdvertisementWatcherReceived = this._onAdvertisementWatcherReceived.bind(
        this
    );
    this._onAdvertisementWatcherStopped = this._onAdvertisementWatcherStopped.bind(
        this
    );
    this._onConnectionStatusChanged = this._onConnectionStatusChanged.bind(
        this
    );
    ...
    this._advertisementWatcher.on(
        'received',
        this._onAdvertisementWatcherReceived
    );
    this._advertisementWatcher.on(
        'stopped',
        this._onAdvertisementWatcherStopped
    );
...
NobleBindings.prototype.startScanning = function(
    ...
    if (
        this._advertisementWatcher.status ===
...
NobleBindings.prototype.connect = function(deviceUuid) {
    ...
    if (!deviceRecord.connectable) {
        throw new Error(
            'Device is not connectable: ' + deviceRecord.formattedAddress
        );
    }

    rt.promisify(BluetoothLEDevice.fromBluetoothAddressAsync)(
        deviceRecord.address
    )
        .then(device => {
            if (device) {
                debug(
                    'got bluetooth device: %s (%s)',
                    device.name,
                    device.deviceInformation.kind
                );
                ...
                this.emit(
                    ...
                    deviceUuid,
                    new Error(
                        'Failed to get a Bluetooth device pointer from address for device UUID: ' +
                            deviceUuid
                    )
                );
            }
...
NobleBindings.prototype.discoverServices = function(
    deviceUuid,
    filterServiceUuids
) {
    ...
    rt.promisify(device.getGattServicesAsync, device)(
        ...
        .then(result => {
            ...
            let services = rt.trackDisposables(
                deviceUuid,
                rt.toArray(result.services)
            );
            let serviceUuids = services
        ...
        .catch(ex => {
            debug(
                'failed to get GATT services for device %s: %s',
                deviceUuid,
                ex.stack
            );
    ...
    this._getCachedServiceAsync(deviceUuid, serviceUuid)
        .then(service => {
            rt.promisify(service.getIncludedServicesAsync, service)(
                ...
                let includedServices = rt.trackDisposables(
                    deviceUuid,
                    rt.toArray(result.services)
                );
        ...
        .catch(ex => {
            debug(
                'failed to get GATT included services for device %s: %s',
                deviceUuid,
                +ex.stack
            );
...
NobleBindings.prototype.discoverCharacteristics = function(
    ...
    this._getCachedServiceAsync(deviceUuid, serviceUuid)
        .then(service => {
            return rt
                .promisify(service.getCharacteristicsAsync, service)(
                    BluetoothCacheMode.uncached
                )
                .then(result => {
                    checkCommunicationResult(deviceUuid, result);
                    ...
                    let characteristics = rt
                        ...
                        .filter(c => {
                            return filterUuids(filterCharacteristicUuids)(
                                uuidToString(c.uuid)
                            );
                        })
                        .map(c => ({
                            uuid: uuidToString(c.uuid),
                            properties: characteristicPropertiesToStrings(
        ...
        .catch(ex => {
            debug(
                'failed to get GATT characteristics for device %s: %s',
                deviceUuid,
                ex.stack
            );
...
NobleBindings.prototype.read = function(
    ...
    this._getCachedCharacteristicAsync(
        ...
        .then(characteristic => {
            return rt
                .promisify(characteristic.readValueAsync, characteristic)(
                    BluetoothCacheMode.uncached
                )
                .then(result => {
                    checkCommunicationResult(deviceUuid, result);
        ...
        .catch(ex => {
            debug(
                'failed to read characteristic for device %s: %s',
                deviceUuid,
                ex.stack
            );
            this.emit(
...
NobleBindings.prototype.write = function(
    ...
    this._getCachedCharacteristicAsync(
        ...
        .catch(ex => {
            debug(
                'failed to write characteristic for device %s: %s',
                deviceUuid,
                ex.stack
            );
            if (!withoutResponse) {
...
NobleBindings.prototype.notify = function(
    ...
    this._getCachedCharacteristicAsync(
        ...
        .catch(ex => {
            debug(
                'failed to enable characteristic notify for device %s: %s',
                deviceUuid,
                ex.stack
            );
            this.emit(
...
NobleBindings.prototype.discoverDescriptors = function(
    ...
    this._getCachedCharacteristicAsync(
        ...
        .then(characteristic => {
            return rt
                .promisify(characteristic.getDescriptorsAsync, characteristic)(
                    BluetoothCacheMode.uncached
                )
                .then(result => {
                    checkCommunicationResult(deviceUuid, result);
                    ...
                    let descriptors = rt
                        .toArray(result.descriptors)
                        .map(d => uuidToString(d.uuid));
                    this.emit(
                        'descriptorsDiscover',
                        deviceUuid,
                        serviceUuid,
        ...
        .catch(ex => {
            debug(
                'failed to get GATT characteristic descriptors for device %s: %s',
                deviceUuid,
                ex.stack
            );
            this.emit(
...
NobleBindings.prototype.readValue = function(
    ...
    return this._getCachedDescriptorAsync(
        ...
        .then(descriptor => {
            return rt
                .promisify(descriptor.readValueAsync, descriptor)(
                    BluetoothCacheMode.uncached
                )
                .then(result => {
                    checkCommunicationResult(deviceUuid, result);
        ...
        .catch(ex => {
            debug(
                'failed to read GATT characteristic descriptor values for device %s: %s',
                deviceUuid,
                ex.stack
            );
            this.emit(
...
NobleBindings.prototype.writeValue = function(
    ...
    this._getCachedDescriptorAsync(
        ...
        .then(descriptor => {
            let rtBuffer = rt.fromBuffer(data);
            return rt
                .promisify(descriptor.writeValueWithResultAsync, descriptor)(
                    rtBuffer
                )
                .then(result => {
                    checkCommunicationResult(deviceUuid, result);
        ...
        .catch(ex => {
            debug(
                'failed to write characteristic descriptor for device %s: %s',
                deviceUuid,
                ex.stack
            );
            if (!withoutResponse) {
...
NobleBindings.prototype._updateRadioState = function() {
    ...
    if (!this._radio) {
        ...
        switch (this._radio.state) {
            ...
            default:
                debug(
                    'bluetooth radio is in unknown state: ' +
                        this._bluetoothRadio.state
                );
                state = 'unknown';
                break;
        }
...
NobleBindings.prototype._onAdvertisementWatcherReceived = function(sender, e) {
    ...
    const isScanResponse =
        e.advertisementType === BluetoothLEAdvertisementType.scanResponse;
    if (isScanResponse) {
        ...
        if (
            !this._allowAdvertisementDuplicates &&
            this._deviceMap[deviceUuid]
        ) {
        ...
        serviceUuids = rt
            .toArray(e.advertisement.serviceUuids)
            .map(serviceUuid => {
                if (debug.enabled) {
                    debug(
                        '	service UUID: %s',
                        getEnumName(GattServiceUuids, serviceUuid) ||
                            uuidToString(serviceUuid)
                    );
                }
                serviceUuid = uuidToString(serviceUuid);
                if (
                    !serviceUuidMatched &&
                    this._filterAdvertisementServiceUuids.indexOf(
                        serviceUuid
                    ) >= 0
                ) {
        ...
        if (!serviceUuidMatched) {
            debug(
                '	Ignoring advertisement that did not pass service UUID filter'
            );
            return;
        }
    }
    ...
    let addressType =
        e.bluetoothAddress >= 3 * Math.pow(2, 46) ? 'random' : 'public';
    ...
    dataSections.forEach(dataSection => {
        ...
        switch (dataSection.dataType) {
            ...
            case BluetoothLEAdvertisementDataTypes.completeService128BitUuids:
                serviceDataEntry.uuid = rt
                    .toBuffer(dataSection.data)
                    .toString('hex');
                break;
    ...
    if (manufacturerSections.size > 0) {
        ...
        deviceRecord.manufacturerData = Buffer.concat([
            toAppend,
            deviceRecord.manufacturerData
        ]);
        debug(
            '	manufacturer data: %s',
            deviceRecord.manufacturerData.toString('hex')
        );
    }
    ...
    if (!this._acceptOnlyScanResponse || isScanResponse) {
        ...
        debug(
            'discover: %s [%s]',
            deviceUuid,
            advertisement.serviceUuids.join()
        );
        this.emit(
            'discover',
            deviceUuid,
...
NobleBindings.prototype._onAdvertisementWatcherStopped = function(sender, e) {
    if (
        ...
        debug(
            'watcher stopped with unexpected status: %s',
            this._advertisementWatcher.status
        );
    }
    this.emit('scanStop');
...
NobleBindings.prototype._getCachedServiceAsync = function(
    deviceUuid,
    serviceUuid
) {
    let deviceRecord = this._deviceMap[deviceUuid];
    ...
    return rt
        .promisify(device.getGattServicesAsync, device)(
...
NobleBindings.prototype._getCachedCharacteristicAsync = function(
    deviceUuid,
    serviceUuid,
    characteristicUuid
) {
    ...
    return this._getCachedServiceAsync(deviceUuid, serviceUuid).then(
        service => {
            return rt
                .promisify(service.getCharacteristicsAsync, service)(
                    BluetoothCacheMode.cached
...
function stringToUuid(uuid) {
    if (typeof uuid === 'number') {
        ...
        return BluetoothUuidHelper.fromShortId(parseInt(uuid, 16)).replace(
            /[{}]/g,
            ''
        );
    } else {
...
function checkCommunicationResult(deviceUuid, result) {
    if (result.status === GattCommunicationStatus.unreachable) {
        ...
        throw new Error(
            'Protocol error communicating with device: ' + deviceUuid
        );
    }
}
